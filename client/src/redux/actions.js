...
export function getDogById(id) {
  return async function (dispatch) {
    try {
      let dogId = await axios.get(`http://localhost:3001/dogs/${id}`);
...
export function searchDog(search) {
  return async function (dispatch) {
    try {
      let dogs = await axios.get(`http://localhost:3001/dogs?name=${search}`);
      return dispatch({
        type: SEARCH_DOG,
        payload: dogs.data,
      });
...
export function postDog(input) {
  return async function (dispatch) {
    try {
      let post = await axios.post(`http://localhost:3001/dogs/`, input);
      console.log(post);
      return dispatch({
        type: POST_DOG,
      });
